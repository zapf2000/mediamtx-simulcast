...
class MediaMTXMoQPublisher {
  ...
  #uniStreamsQueue = [];
  #uniStreamsListeners = [];
  #videoEncoders = [];  // simulcast encoders
  #audioEncoder = null;
  ...
  #cleanup() {
    ...
    for (const enc of this.#videoEncoders) {
  ...
  #getVideoLayers() {
    if (this.#conf.videoLayers && this.#conf.videoLayers.length > 0) {
      return this.#conf.videoLayers;
    }
    return [{ width: this.#conf.videoWidth, height: this.#conf.videoHeight, bitrate: this.#conf.videoBitrate }];
  }
  ...
  #buildCatalog() {
    ...
    if (videoTracks.length > 0) {
      for (const layer of this.#getVideoLayers()) {
        tracks.push({
  ...
  async #publishAllTracks() {
    ...
    const videoTracks = this.#conf.stream.getVideoTracks();
    if (videoTracks.length > 0) {
      const layers = this.#getVideoLayers();
      for (let i = 0; i < layers.length; i++) {
        promises.push(this.#encodeVideo(videoTracks[0], alias, layers[i], i));
        alias++;
      }
    }
  ...
  async #encodeVideo(mediaTrack, trackAlias, layer, encoderIndex) {
    if (!layer) layer = { width: this.#conf.videoWidth, height: this.#conf.videoHeight, bitrate: this.#conf.videoBitrate };
    if (encoderIndex === undefined) encoderIndex = 0;
    ...
    this.#videoEncoders[encoderIndex] = encoder;
    const videoConfig = {
      codec: this.#conf.videoCodec,
