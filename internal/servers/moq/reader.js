...
class MediaMTXMoQReader {
  ...
  static #VIDEO_REQUEST_ID_BASE = BigInt(10); // 10, 12, 14, ... for video layers
  static #AUDIO_REQUEST_ID = BigInt(11);
  static #VIDEO_REQUEST_ID = BigInt(10); // kept for backward compat
  ...
  #uniStreamsListeners = [];
  #videoTrack = null;
  #videoTracks = [];  // all video tracks from catalog
  ...
  /**
   * Get all available video tracks from catalog.
   * @returns {Array} array of video track objects
   */
  getVideoTracks() {
  ...
  /**
   * Switch to a different video layer by reconnecting.
   * @param {number} index - layer index (0 = highest quality)
   */
  selectLayer(index) {
  ...
  async #subscribeAllTracks() {
    const promises = [];
    const videoLayer = this.#selectedLayer;
    ...
    // Subscribe ALL video layers with unique request IDs
    if (this.#videoTracks.length > 0) {
      const initialLayer = Math.min(videoLayer, this.#videoTracks.length - 1);
      this.#currentVideoLayer = initialLayer;
      this.#videoTrack = this.#videoTracks[initialLayer];
      this.#videoTrackAliases = [];
      for (let vi = 0; vi < this.#videoTracks.length; vi++) {
        const alias = MediaMTXMoQReader.#VIDEO_REQUEST_ID_BASE + BigInt(vi * 2);
        this.#videoTrackAliases.push(alias);
        promises.push(
          this.#subscribeVideoLayer(alias, this.#videoTracks[vi], vi === initialLayer),
        );
      }
    }
    ...
    if (this.#conf.onSubscribed !== undefined) {
      this.#conf.onSubscribed(this.#audioTrack !== null, this.#videoTracks);
    }
  }

  // Subscribe a video layer - creates decoder only for active layer
  async #subscribeVideoLayer(requestId, track, isActive) {
    const bidi = await this.#wt.createBidirectionalStream();
    const w = bidi.writable.getWriter();
    const r = new MediaMTXMoQReader.#StreamReader(bidi.readable);
    ...
    if (isActive) {
      this.#setupVideoDecoder(track);
    }

    console.log("subscribed video layer " + track.name + " alias=" + requestId + (isActive ? " [active]" : ""));
  }

  #setupVideoDecoder(track) {
    // Clean up old decoder/canvas
    if (this.#videoDecoder !== null) {
    ...
    if (ctxGL === null) {
      ...
      ctxGL.shaderSource(vert, `#version 300 es
      ...
      ctxGL.shaderSource(frag, `#version 300 es
        precision mediump float;
        uniform sampler2D u_tex; in vec2 v_uv; out vec4 color;
      ...
      ctxGL.bufferData(ctxGL.ARRAY_BUFFER,
        new Float32Array([-1,-1, 1,-1, -1,1, 1,1]), ctxGL.STATIC_DRAW);
      ...
      ctxGL.texParameteri(ctxGL.TEXTURE_2D, ctxGL.TEXTURE_MIN_FILTER, ctxGL.LINEAR);
      ctxGL.texParameteri(ctxGL.TEXTURE_2D, ctxGL.TEXTURE_MAG_FILTER, ctxGL.LINEAR);
      ctxGL.texParameteri(ctxGL.TEXTURE_2D, ctxGL.TEXTURE_WRAP_S, ctxGL.CLAMP_TO_EDGE);
      ctxGL.texParameteri(ctxGL.TEXTURE_2D, ctxGL.TEXTURE_WRAP_T, ctxGL.CLAMP_TO_EDGE);
      ...
      output: (frame) => {
        try {
          if (ctxGL !== null) {
            ...
            ctxGL.viewport(0, 0, frame.displayWidth, frame.displayHeight);
            ctxGL.texImage2D(ctxGL.TEXTURE_2D, 0, ctxGL.RGBA, ctxGL.RGBA,
              ctxGL.UNSIGNED_BYTE, frame);
  ...
  async #onDataTrack(readable) {
    ...
    // Check if this is the currently active video layer
    const activeVideoAlias = this.#videoTrackAliases.length > 0
      ? this.#videoTrackAliases[this.#currentVideoLayer]
      : MediaMTXMoQReader.#VIDEO_REQUEST_ID;
    const isVideoAlias = this.#videoTrackAliases.length > 0
      ? this.#videoTrackAliases.includes(trackAlias)
    ...
    if (isVideoAlias) {
      ...
      // else: inactive video layer - discard
    } else {
  ...
  #decodeVideo(data, groupId) {
    // Skip if decoder not ready (e.g. during layer switch)
    if (this.#videoDecoder === null || this.#videoDecoder.state === "closed") {
      return;
    }
    // After layer switch, wait for keyframe before decoding
    if (this.#waitingForKeyframe) {
      // Check if this is a keyframe (has SPS for H264, or VPS for H265)
      let isKeyframe = false;
      if (this.#videoTrack !== null && /^avc3/.test(this.#videoTrack.codec)) {
        for (const nalu of MediaMTXMoQReader.#splitAVCC(data)) {
          if ((nalu[0] & 0x1f) === 7) { isKeyframe = true; break; }
        }
      } else if (this.#videoTrack !== null && /^hev1/.test(this.#videoTrack.codec)) {
        for (const nalu of MediaMTXMoQReader.#splitAVCC(data)) {
          if ((nalu[0] & 0x7e) >> 1 === 32) { isKeyframe = true; break; }
        }
      } else {
        isKeyframe = true; // for other codecs, assume keyframe
      }
